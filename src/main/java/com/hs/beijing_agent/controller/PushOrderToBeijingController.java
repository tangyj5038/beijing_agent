...
public class PushOrderToBeijingController extends BaseController{


    private static HttpClientTemplate client = HttpClientTemplate.Factory.getClient();

    /**
     *@描述 1-获取单个订单的推送报文信息，处于测试阶段
     *@参数
     *@返回值
     *@创建人  TangYujie
     *@创建时间  2019/7/2 15:15
     */
    @RequestMapping(value="/message/single/{orderId}",method = RequestMethod.GET,produces = {JSON_UTF8})
    public String  getSingleOrderMessage(@PathVariable(value="orderId",required = true)Long orderId) throws IOException {
        //1-请求订单Xml数据
        String receiveUrl = Constant.SERVER_IP + "api/beijing/customer/get/orderInfo/again/" + orderId;
